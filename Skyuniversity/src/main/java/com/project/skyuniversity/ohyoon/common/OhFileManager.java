	...
	public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response) {
		...
		try {
			...
			if (file.exists()) { // 실제로 다운로드할 해당 파일이 존재한다면
				...
				byte[] readByte = new byte[4096]; // 4kb의 byte 배열을 생성.
				// 다운로드할 파일의 내용을 읽어오는 단위크기를 4096 byte로 하는 byte 배열 readByte를 생성한다. 
				...
				ServletOutputStream souts = response.getOutputStream(); // 웹상에서 파일을 보내줄 창구
				// ServletOutputStream은 다운로드 되어질 파일을 클라이언트로 보내어 주는 출력 스트림용 클래스이다.
				
				int length = 0;
				
				while ( (length = bfin.read(readByte, 0, 4096)) != -1 ) { // 처음(0)부터 4096byte 크기씩 읽어 readByte에 넣는 것. 더이상 읽을 것이 없으면 read()는 -1값을 리턴한다. 
				/*
					bfin.read(readByte, 0, 4096)은 다운로드 해주어야할 file에서 0(처음)부터 4096 byte 만큼 읽어들인 후, 
					readByte 라는 변수에  읽어들인 내용을 저장시킨다.
					그런데 다운로드 해주어야할 file에서 읽어들인 내용이 없다면
					bfin.read(readByte, 0, 4096)은 -1값을 리턴한다.
					그러므로 while ( (length = bfin.read(readByte, 0, 4096)) != -1 )라는 말은
					다운로드 해주어야 할 file을 매번 4096byte씩 읽어들인다는 말이다.
				*/
				
				souts.write(readByte, 0, length);	
				/*
					readByte에 저장된 내용을 처음부터 읽어온 크기인 length 만큼 
					ServletOutputStream souts에 기록(저장)해둔다.
				*/
					
				}// end of while()---------------------------------------------- 
				
				souts.flush(); // ServletOutputStream souts에 기록(저장)해둔 내용을 클라이언트로 내보내준다.
				
				souts.close(); // ServletOutputStream souts를 다썼으므로 메모리상에서 소멸시킨다.
				bfin.close(); // BufferedInputStream bfin를 다썼으므로 메모리상에서 소멸시킨다.
				
				return true; // 다운로드 해줄 파일이 존재하고 Exception이 발생하지 않으면 true를 리턴시켜준다.
