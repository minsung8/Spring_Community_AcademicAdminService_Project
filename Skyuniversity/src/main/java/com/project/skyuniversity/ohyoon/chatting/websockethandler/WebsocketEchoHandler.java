package com.project.skyuniversity.ohyoon.chatting.websockethandler;
...
public class WebsocketEchoHandler extends TextWebSocketHandler {
       ...
       */
       ...
       /*
        ...
        for (WebSocketSession webSocketSession : connectedUsers) {
            ...
            */ 
           CommuMemberVO loginuser = (CommuMemberVO)map.get("loginuser");  
           // "loginuser" 은 HttpSession에 저장된 키 값으로 로그인 되어진 사용자이다.
            connectingUserName += loginuser.getNickname()+" "; 
        }
        connectingUserName += "」";
   ...
   // === 클라이언트가 웹소켓 서버로 메시지를 보냈을때의 Send 이벤트를 처리하기 ===
    ...
    protected void handleTextMessage(WebSocketSession wsession, TextMessage message) 
       ...
       System.out.println("====> 웹채팅확인용 : 로그인ID : " + loginuser.getFk_memberNo());
       // ====> 웹채팅확인용 : 로그인ID : seoyh
    ...
    public void afterConnectionClosed(WebSocketSession wsession, CloseStatus status) 
       ...
       // 웹소켓 서버에 연결되어진 클라이언트 목록에서 연결은 끊은 클라이언트는 삭제시킨다.
        ...
        for (WebSocketSession webSocketSession : connectedUsers) {
           ...
           if (!wsession.getId().equals(webSocketSession.getId())) { 
                webSocketSession.sendMessage(
                   new TextMessage(wsession.getRemoteAddress().getAddress().getHostAddress() +" [<span style='font-weight:bold;'>" +loginuser.getNickname()+ "</span>]" + "님이 <span style='color: red;'>퇴장</span>했습니다.")
                ); 
            }
        }
        ...
        for (WebSocketSession webSocketSession : connectedUsers) {
            ...
            CommuMemberVO loginuser2 = (CommuMemberVO)map2.get("loginuser");  
           // "loginuser" 은 HttpSession에 저장된 키 값으로 로그인 되어진 사용자이다.
   
            connectingUserName += loginuser2.getNickname()+" "; 
        }
