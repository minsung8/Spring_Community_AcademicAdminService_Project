package com.project.skyuniversity.minsung.common;
...
public class FileManager {
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		// pathname 은 예를 들면,
		// C:\NCS\workspace(spring)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\Board\resources\files\2020120809271535243254235235234.png
		// 이다.

		FileOutputStream fos = new FileOutputStream(pathname);
		// FileOutputStream 는 해당 경로 파일명(pathname)에 실제로 데이터 내용(byte[] bytes)을 기록해주는 클래스
		// 이다.
		// 이러한 일을 하는 FileOutputStream 객체 fos 를 생성한다.

		fos.write(bytes);
		// write(byte[] bytes) 메소드가 해당 경로 파일명(pathname)에 실제로 데이터 내용(byte[] bytes)을 기록해주는
		// 일을 하는 것이다.
	...
	// === 파일 다운로드 하기 === //
	// saveFilename : 서버에 저장된 파일명(현재의 년월일시분초에다가 현재 나노세컨즈nanoseconds 값을 결합하여 확장자를 붙여서
	// 만든것)
	// originalFilename : 클라이언트가 업로드한 파일명(파일명이 영어로 되어진 경우도 있지만 한글로 되어진 경우가 있다는 것에
	// 유의하자)
	// path : 서버에 저장된 경로
	public boolean doFileDownload(String saveFilename, String originalFilename, String path,
		...
		try {
			...
			// originalFilename.getBytes("UTF-8") 은 UTF-8 형태로 되어진 문자열 originalFilename 을
			// byte 형태로 변경한 후
		...
		try {
			...
			// 다운로드 할 파일명(pathname)을 가지고 file 객체 생성
			...
			if (file.exists()) { // 실제로 다운로드할 해당 파일이 존재한다면
				...
				// 클라이언트가 다운로드 해야 할 파일을 읽어들이기 위해 new FileInputStream(file)을 생성함
				// 또한 빠르게 읽어들이기 위해서 필터스트림(보조스트림) BufferedInputStream bfin을 장착함

				ServletOutputStream souts = response.getOutputStream();
				// 다운로드 되어질 파일을 클라이언트로 보내어주는 출력 스트림용 클래스

				int length = 0;

				while ((length = bfin.read(readByte, 0, 4096)) != -1) {
					/*
					 * bfin.read(readByte, 0, 4096) 은 다운로드 해주어야할 file에서 0(처음) 부터 4096 byte 만큼 읽어들인 후
					 * readByte 라는 변수에 읽어들인 내용을 저장시킨다. 그런데 다운로드 해주어야할 file에서 읽어들인 내용이 없다라면
					 * bfin.read(readByte, 0, 4096) 은 -1 을 리턴시켜준다. 그러므로 while( (length =
					 * bfin.read(readByte, 0, 4096)) != -1 ) 말은 다운로드 해주어야할 file 을 매번 4096 byte 씩
					 * 읽어들인다는 말이다.
					 */
					souts.write(readByte, 0, length);
					/*
					 * readByte에 저장된 내용을 처음부터 읽어온 크기인 length 만큼 ServletOutputStream souts에 기록
					 */
				}
				souts.flush();
